-- FULLSEND V16.7 — ficha técnica profissional de anúncios
-- Adiciona campos reais ao anúncio FULLSEND e sincroniza com busca/filtros públicos.
...
alter table public.listings
  ...
  add column if not exists vehicle_styles text[] not null default '{}',
...
create index if not exists listings_vehicle_search_idx
  on public.listings (status, category_slug, brand, model, year, created_at desc);

create index if not exists listings_vehicle_styles_gin_idx
  on public.listings using gin (vehicle_styles);
...
unified as (
  select
    ...
    jsonb_build_object('id',pr.id,'name',pr.name,'avatar_url',pr.avatar_url,'badge',pr.badge) as seller
