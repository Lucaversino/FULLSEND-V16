-- FULLSEND V17.2.1 — reparo reaplicável. Preserva publicações e contas.
...
-- Privilégios explícitos usados pela função SECURITY INVOKER, sempre sob RLS.
grant usage on schema public to anon,authenticated;
grant select(id,name,avatar_url,city,state) on public.profiles to anon,authenticated;
grant select(id,user_id,title,slug,status) on public.listings to anon,authenticated;
grant select(id,title,slug,event_date,event_time,city,state,venue,status,created_by) on public.events to anon,authenticated;
