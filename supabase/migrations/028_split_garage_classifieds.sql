-- FULLSEND V17.2.4
-- Separa veículos da garagem pessoal dos anúncios classificados.
-- Todos os registros antigos continuam como classificados.
...
where listing_mode is null or listing_mode not in ('classified','garage');
...
-- Recria a busca pública vigente da V17.1, acrescentando somente a exclusão
-- de carros cadastrados como Minha Garagem.
-- FULLSEND V17.1
-- A home passa a poder sortear anúncios de toda a base com uma seed diferente
-- a cada novo carregamento, mantendo a mesma ordem durante a paginação.
--
-- p_sort continua com a mesma assinatura.
-- Formato novo: random:<seed>
-- Compatibilidade: "random" antigo continua funcionando.
...
unified as (
  ...
  where l.status='active' and coalesce(l.listing_mode,'classified')='classified'
  ...
  select f.*,
    row_number() over (
      order by
        ...
        case
          when (select sort_q from params) like 'random:%'
            then md5(f.id || ':' || (select sort_q from params))
          when (select sort_q from params)='random'
            then md5(f.id)
        end asc,
