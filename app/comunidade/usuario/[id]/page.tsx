...
export default async function Page({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{pagina?:string}>}){
 ...
 <div className="fs-profile-identity"><div className="fs-profile-avatar-stack"><div className="fs-profile-avatar">{safeImage(p.avatar_url)?<img src={safeImage(p.avatar_url)} alt={`Foto de ${p.name||'membro'}`}/>:(p.name||'F').slice(0,1)}</div><UserBadge badge={p.role==='admin'?'admin':p.badge}/></div><div className="fs-profile-name"><span className="fs-profile-kicker">MEMBRO FULLSEND</span><div className="fs-profile-name-line"><h1>{p.name||'Membro FULLSEND'} <VerifiedBadge active={p.is_verified}/></h1><div className="fs-profile-badges"><ReputationBadge level={p.reputation_level} xp={p.xp_points}/></div></div>{location&&<p>{location}</p>}</div><div className="fs-profile-actions">{owner?<Link href="/perfil">Editar meu perfil</Link>:<FollowUserButton userId={id}/>}<a href="#garagem">Ver garagem ↓</a></div></div>
