...
export default async function Page({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{pagina?:string}>}){
 ...
 const [{data:p,error},{data:{user}},counts,cars]=await Promise.all([s.from('profiles').select('id,name,avatar_url,city,state,bio').eq('id',id).maybeSingle(),s.auth.getUser(),s.rpc('community_follow_counts',{target:id}),s.from('listings').select('id,title,cover_url').eq('user_id',id).eq('listing_mode','garage').eq('status','active').order('created_at',{ascending:false}).range((page-1)*12,page*12)])
