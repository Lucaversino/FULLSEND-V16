...
export default async function Page({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{pagina?:string}>}){
 ...
 const [{data:p,error},{data:{user}},counts,cars]=await Promise.all([s.from('profiles').select('id,name,avatar_url,city,state,bio').eq('id',id).maybeSingle(),s.auth.getUser(),s.rpc('community_follow_counts',{target:id}),s.from('listings').select('id,title,cover_url,brand,model,year,power_cv',{count:'exact'}).eq('user_id',id).eq('listing_mode','garage').eq('status','active').order('created_at',{ascending:false}).range((page-1)*12,page*12)])
 if(error)throw new Error('Não foi possível carregar o perfil.');if(!p)notFound()
 const owner=user?.id===id
 const location=[p.city,p.state].filter(Boolean).join(' / ')
 return <div className="fs-public-profile"><section className="fs-profile-shell">
 <div className="fs-profile-cover"><span>FULLSEND COMMUNITY</span><strong>SUA GARAGEM.<br/><em>SUA IDENTIDADE.</em></strong><small>CARROS CONECTAM PESSOAS.</small></div>
 <div className="fs-profile-identity"><div className="fs-profile-avatar">{safeImage(p.avatar_url)?<img src={safeImage(p.avatar_url)} alt={`Foto de ${p.name||'membro'}`}/>:(p.name||'F').slice(0,1)}</div><div className="fs-profile-name"><span className="fs-profile-kicker">MEMBRO FULLSEND</span><h1>{p.name||'Membro FULLSEND'}</h1>{location&&<p>{location}</p>}</div><div className="fs-profile-actions">{owner?<Link href="/perfil">Editar meu perfil</Link>:<FollowUserButton userId={id}/>}<a href="#garagem">Ver garagem ↓</a></div></div>
