...
export async function PATCH(req:Request){
  ...
  const profilePayload={name:String(body.name||'').trim(),city:String(body.city||'').trim(),state:String(body.state||'').trim().toUpperCase(),whatsapp:String(body.whatsapp||'').trim(),badge:body.role==='admin'?'admin':'none',role:body.role,account_status:body.account_status,last_admin_note:String(body.last_admin_note||'').trim(),updated_at:new Date().toISOString()}
  ...
  await gate.admin.from('audit_logs').insert({actor_id:gate.user.id,action:'admin_user_update',entity:'user',entity_id:id,data:{badge:body.role==='admin'?'admin':'none',role:body.role,status:body.account_status}})
