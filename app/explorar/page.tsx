...
function FilterFields({ p, base }:{ p:Record<string,string|undefined>, base:string }) {
  return <>
    ...
    <div className="filter-group filter-style-visual">
      <label>Estilo</label>
      <StyleBannerButtons params={p} base="/explorar" compact />
    </div>
    ...
    <Link href="/explorar" className="filter-clear"><RotateCcw size={14}/> LIMPAR FILTROS</Link>
  </>
}
...
function FilterSidebar({ p }:{ p:Record<string,string|undefined> }) {
  return <aside className="filters-sidebar">
    <div className="filters-card desktop-filters">
      ...
      <form method="GET" action="/explorar"><FilterFields p={p} base="/explorar"/></form>
    </div>
...
export default async function Explorar({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <main className="section explore-page">
    <div className="container">
      <div className="page-head">
        <span className="section-kicker">FULLSEND MARKET</span>
        <h1>EXPLORAR</h1>
        <p>Busque projetos, carros preparados, peças e performance.</p>
      </div>
      <SearchBar target="/explorar" initialQuery={p.q || ''} initialCategory={p.categoria || ''} initialState={p.estado || ''} initialCity={p.cidade || ''}/>
        ...
        <section className="explore-results">
          ...
          {totalResults ? <>
            ...
            {totalPages > 1 ? <nav className="manual-pagination" aria-label="Paginação dos anúncios">
              <Link className={`page-arrow ${currentPage === 1 ? 'disabled' : ''}`} aria-disabled={currentPage===1} href={pageHref('/explorar',p,Math.max(1,currentPage-1))}>‹</Link>
              <div className="page-numbers">
                {pages.map((n,i)=>{
                  const prev = pages[i-1]
                  return <span key={n} className="page-slot">
                    {prev && n-prev>1 ? <span className="page-ellipsis">…</span> : null}
                    <Link className={`page-number ${n===currentPage?'active':''}`} href={pageHref('/explorar',p,n)}>{n}</Link>
                  </span>
                })}
              </div>
              <Link className={`page-arrow ${currentPage === totalPages ? 'disabled' : ''}`} aria-disabled={currentPage===totalPages} href={pageHref('/explorar',p,Math.min(totalPages,currentPage+1))}>›</Link>
            </nav> : null}
