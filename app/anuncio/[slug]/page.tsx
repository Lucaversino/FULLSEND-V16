...
export default async function Anuncio({params}:{params:Promise<{slug:string}>}){
  ...
  const {data:seller}=x.user_id?await s.from('profiles').select('name,avatar_url,badge,city,state,bio,xp_points,reputation_level,is_verified').eq('id',x.user_id).maybeSingle():{data:null as any}
