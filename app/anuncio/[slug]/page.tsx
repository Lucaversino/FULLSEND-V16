...
export default async function Anuncio({params}:{params:Promise<{slug:string}>}){
  const {slug}=await params;const s=await createClient();const {data:x}=await s.from('listings').select('*').eq('slug',slug).eq('status','active').maybeSingle();if(!x)notFound()
  const {data:seller}=x.user_id?await s.from('profiles').select('name,avatar_url,badge,city,state,bio').eq('id',x.user_id).maybeSingle():{data:null as any}
...
</div>:null}{seller?<div className="native-seller-card"><div className="native-seller-avatar">{seller.avatar_url?<img src={seller.avatar_url} alt=""/>:<span>{String(seller.name||'F')[0].toUpperCase()}</span>}</div><div><small>ANUNCIANTE</small><strong>{seller.name||'Membro FULLSEND'}</strong><UserBadge badge={seller.badge}/></div></div>:null}{seller&&x.user_id?<DirectMessageButton recipientId={x.user_id} listingId={x.id} listingTitle={x.title}/>:null}{wa?<a className="btn btn-red native-wa" href={`https://wa.me/55${wa}?text=${txt}`} target="_blank" rel="noopener noreferrer"><MessageCircle size={17}/> CHAMAR NO WHATSAPP</a>:null}</aside></div></div></main>
}
