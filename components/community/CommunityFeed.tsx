...
export default function CommunityFeed({author,vehicle,tag,postId,saved=false}:{author?:string;vehicle?:string;tag?:string;postId?:string;saved?:boolean}){
 const mainFeed=!postId&&!saved&&!vehicle&&!author&&!tag
 ...
 return <div className="cm-root">
  ...
  <div className="cm-layout"><aside className="cm-sidebar"><span className="cm-eyebrow">CULTURA AUTOMOTIVA</span><h2>Sua garagem.<br/>Sua história.</h2><p>Projetos, encontros e conversas de quem vive o mundo automotivo.</p><button className="cm-primary" onClick={create} disabled={sessionBusy}><Plus size={17}/> Criar publicação</button><Link className="cm-side-link" href="/perfil">Abrir minha garagem →</Link><Link className="cm-side-link" href="/comunidade/salvos">Ver publicações salvas →</Link><div className="cm-side-note">Compartilhe conhecimento. Respeite outros membros. Denuncie golpes e conteúdo inadequado.</div></aside>
  <section className={`cm-feed${mainFeed?' cm-feed-main':''}`}>
   {!mainFeed&&<div className="cm-row cm-feed-title"><h2>{postId?'Publicação':tag?`#${tag}`:saved?'Publicações salvas':vehicle?'Diário e publicações do projeto':author?'Publicações do membro':'Na comunidade'}</h2><button onClick={refresh} disabled={loading} aria-label="Atualizar feed"><RefreshCw size={16}/></button></div>}
   {!postId&&<div className="cm-search-row"><form className="cm-search" onSubmit={e=>{e.preventDefault();setQuery(input);setPage(1)}}><Search size={18}/><input aria-label="Buscar na Comunidade" value={input} onChange={e=>setInput(e.target.value)} maxLength={100} placeholder="Busque pessoas, carros, projetos e hashtags"/><button>Buscar</button></form>{mainFeed&&<button className="cm-refresh" onClick={refresh} disabled={loading} aria-label="Atualizar feed"><RefreshCw size={16}/></button>}</div>}
  ...
  <footer className="cm-community-footer"><UserDiscovery/><p>Encontre membros, conheça novas garagens e acompanhe projetos da Comunidade FULLSEND.</p></footer>
 </div>
}
