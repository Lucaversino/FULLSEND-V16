...
export default function CommunityFeed({author,vehicle,tag,postId,saved=false}:{author?:string;vehicle?:string;tag?:string;postId?:string;saved?:boolean}){
 ...
 useEffect(()=>{let active=true;setLoading(true);setError('');if(page===1)setPosts([])
  ...
  request(`/api/community?${params}`).then(d=>{if(!active)return;setPosts(old=>page===1?d.items:Array.from(new Map([...old,...d.items].map((x:any)=>[x.id,x])).values()));setMore(d.hasMore);setViewer(d.viewer);setIsAdmin(d.isAdmin);if(page===1){setPeople(d.people);setCars(d.cars)}}).catch(e=>{if(active)setError(e.message)}).finally(()=>{if(active)setLoading(false)})
  return()=>{active=false}
 },[author,vehicle,tag,postId,filter,page,query,location,tick])
 function create(){if(!viewer){window.location.href='/login?next='+encodeURIComponent(window.location.pathname);return}setComposing(true)}
 return <div className="cm-root">
  ...
  <div className="cm-layout"><aside className="cm-sidebar"><span className="cm-eyebrow">CULTURA AUTOMOTIVA</span><h2>Sua garagem.<br/>Sua história.</h2><p>Projetos, encontros e conversas de quem vive o mundo automotivo.</p><button className="cm-primary" onClick={create}><Plus size={17}/> Criar publicação</button><Link className="cm-side-link" href="/perfil">Abrir minha garagem →</Link><Link className="cm-side-link" href="/comunidade/salvos">Ver publicações salvas →</Link><div className="cm-side-note">Compartilhe conhecimento. Respeite outros membros. Denuncie golpes e conteúdo inadequado.</div></aside>
  <section className="cm-feed"><div className="cm-row cm-feed-title"><h2>{postId?'Publicação':tag?`#${tag}`:saved?'Publicações salvas':vehicle?'Diário e publicações do projeto':author?'Publicações do membro':'Na comunidade'}</h2><button onClick={refresh} disabled={loading} aria-label="Atualizar feed"><RefreshCw size={16}/></button></div>
   ...
   <button className="cm-mobile-create cm-primary" onClick={create}><Plus size={18}/> Criar publicação</button>
   {composing&&<Composer vehicleId={vehicle} onClose={()=>setComposing(false)} onDone={()=>{setComposing(false);refresh()}}/>}
   {(people.length>0||cars.length>0)&&<div className="cm-card cm-search-results">{people.length>0&&<><h3>Pessoas</h3>{people.map(p=><Link key={p.id} href={`/comunidade/usuario/${p.id}`}>{p.name||'Membro FULLSEND'} <small>{p.city}</small></Link>)}</>}{cars.length>0&&<><h3>Carros e projetos</h3>{cars.map(c=><Link key={c.id} href={`/comunidade/projeto/${c.id}`}>{c.title} →</Link>)}</>}</div>}
   ...
   {!loading&&!error&&!posts.length&&<div className="cm-card cm-empty"><h3>{postId?'Publicação indisponível':'Nenhuma publicação por aqui ainda'}</h3><p>{postId?'O conteúdo pode ter sido removido ou ocultado.':query?'Tente outro termo de busca.':saved?'As publicações que você salvar aparecerão aqui.':'Compartilhe uma foto, uma dúvida ou a evolução do seu carro.'}</p>{!postId&&!saved&&<button className="cm-primary" onClick={create}>Criar publicação</button>}</div>}
