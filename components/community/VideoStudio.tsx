 ...
 return <div className="fs-figma-backdrop" role="dialog" aria-modal="true" aria-label="FULLSEND Video Studio"><section className="fs-figma-studio">
  ...
  <section className="fs-mobile-timeline-editor" aria-label="Linha do tempo do vídeo">
   ...
   {tracks.length>0&&<div className="fs-mobile-music-section">
    <div className="fs-mobile-music-lane-label"><strong>♫ MÚSICA</strong><span>{tracks.length} FAIXA{tracks.length===1?'':'S'}</span></div>
    <div ref={musicLaneRef} className="fs-mobile-music-lane">
     {tracks.map(track=>{const len=musicLength(track),left=editDuration?track.start/editDuration*100:0,width=editDuration?Math.min(len,Math.max(0,editDuration-track.start))/editDuration*100:0;return <div key={track.id} className={`fs-mobile-music-clip ${activeMusicId===track.id?'active':''}`} style={{left:`${left}%`,width:`${Math.max(3,width)}%`}}
      ...
      onPointerMove={e=>{const d=musicDragRef.current;if(!d||d.id!==track.id||!e.currentTarget.hasPointerCapture(e.pointerId)||!musicLaneRef.current)return;const px=Math.max(1,musicLaneRef.current.getBoundingClientRect().width),delta=(e.clientX-d.startX)/px*editDuration;moveMusic(track.id,d.originalStart+delta)}}
