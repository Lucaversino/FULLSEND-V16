...
export default function VideoStudio({file,onCancel,onConfirm}:{file:File;onCancel:()=>void;onConfirm:(file:File)=>void}){
 ...
 async function playPreview(){const v=videoRef.current;if(!v||!active||!v.paused)return;let editTime=timelineCurrent;if(current<active.start||current>=active.end-.03){seek(active.start);editTime=clipOffset(activeSegment)}v.playbackRate=speed;v.volume=volume;v.muted=muted;armPreviewMusic(editTime);await v.play().catch(()=>{stopPreviewMusic();setError('O navegador bloqueou a reprodução. Toque novamente.')})}
 function pausePreview(){const v=videoRef.current;if(v&&!v.paused)v.pause();stopPreviewMusic();setPlaying(false)}
 function restartPreview(){pausePreview();const first=segments[0];setActiveSegment(0);seek(first?.start??0);syncPreviewMusic(0,false)}
 async function toggle(){if(videoRef.current?.paused)await playPreview();else pausePreview()}
 ...
 return <div className="fs-figma-backdrop" role="dialog" aria-modal="true" aria-label="FULLSEND Video Studio"><section className="fs-figma-studio">
  ...
  <section className="fs-mobile-player" aria-label="Player do vídeo">
   <button type="button" aria-label="Voltar ao início" onClick={restartPreview}>⏮</button>
   <button type="button" aria-label="Voltar 5 segundos" onClick={()=>jump(-5)}>−5</button>
   <button type="button" className="play" aria-label="Reproduzir" onClick={playPreview}><Play size={18}/></button>
   <button type="button" aria-label="Pausar" onClick={pausePreview}><Pause size={16}/></button>
   <button type="button" aria-label="Avançar 5 segundos" onClick={()=>jump(5)}>+5</button>
   <span>{fmt(timelineCurrent)} / {fmt(editDuration)}</span>
